...
remove_donor9 <- as.logical(commandArgs(TRUE)[6])
...
if(remove_donor9){
  ...
  norm_data <- counts_data
}

# select the most variable DARs
norm_data_subset <- norm_data[diff_res$IDs[diff_peaks] , ]
rvdm = apply(norm_data_subset, 1, var)
norm_data_subset = norm_data_subset[order(rvdm, decreasing = T)[1:1000] ,]

# One cluster up one cluster down --------------------------------------------------------
...
pdf(paste0(output_path,"peak_clustering_heatmap.pdf"), w= 10, h=10)
...
Heatmap(norm_data_subset[order(cluster_assignment), sorted_indices] , show_row_names = F,
        left_annotation = ha, top_annotation = col_an, #col = col_fun,
        show_row_dend = F, cluster_rows = F, cluster_columns = F)

col_fun = circlize::colorRamp2(c(-3, -1.5, 0 ,1.5, 3), c("#1F5FA9", "#74C4EA","white","#F4BA58","#A03124"))
