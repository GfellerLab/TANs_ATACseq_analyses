...
KP_DAR_path <- as.character(commandArgs(TRUE)[4])
...
if(grepl("KP_lung_SiglecF_high", KP_DAR_path)){
  comp = "KP_lung_SiglecF_high_VS_KP_lung_SiglecF_low"
  logFC = 2
}else{
  comp = "Nfatc1KO_VS_WT"
  logFC = 1
...
for(DEG_direction in c("up", "down")){
  ...
  if(DEG_direction == "up"){
    diff_genes <- diff_genes[diff_genes$log2FoldChange>0, ]
  }else{
    diff_genes <- diff_genes[diff_genes$log2FoldChane<0, ]
  }
  ...
  for(DAR_direction in c("up", "down")){
    ...
    cluster_peaks <- paste0(KP_DAR_path, "/", comp,"_", DAR_direction, "_logFC",logFC,".txt")
    peaks <- read.table(cluster_peaks, header = T, sep = "\t")
    ...
    # empirical_p <- (sum(rand_counts >= observed_count) + 1) / (1000 + 1)
    # print(empirical_p)
...
pdf(paste0(output_path, "/DARs_DEGs_overlap_window", window_size, ".pdf"), w = 11, h = 5)
...
write.table(summary_subset, file = paste0(output_path, "/DARs_DEGs_overlap_window", window_size, ".txt"), col.names = T, row.names = F, quote = F, sep = "\t")
...
barplot_colors <- c("RNA_genes_up" = "#FF6666", "RNA_genes_down" = "#0066FF", "Rnadom_genes_up" = "gray", "Rnadom_genes_down" = "gray")
...
pdf(paste0(output_path, "/clusterPeaks_DEGs_overlap_window", window_size, ".pdf"), w = 11, h = 5)
...
write.table(summary_subset, file = paste0(output_path, "/clusterPeaks_DEGs_overlap_window", window_size, ".txt"), col.names = T, row.names = F, quote = F, sep = "\t")
