...
get_TF_heatmap <- function(gene_list, genes_matrix, metadata, mat.name = "Deviation score", col_fun = NULL,
  ...
  split.vector <- factor(metadata[colnames(genes_matrix), "groups"], levels = c("Healthy_blood", "KP_blood", "Healthy_lung", "KP_lung_SiglecF_low", "KP_lung_SiglecF_high"))
...
footprinting_res_path2 <- as.character(commandArgs(TRUE)[6])
output_path <- as.character(commandArgs(TRUE)[7])
...
descriminative_colors = c("Healthy_blood"="#3288bdff",
                          ...
                          "batch1" = "#a0451fff",
                          "batch2" = "black", "batch3" = "gray", "batch4" = "#984ea3ff", "batch5" = "yellow4")
...
Top_chromVAR <- rownames(chromvar_mat[order(rvdm, decreasing = T)[1:200] ,])

# Heatmap with avreaged signal per TF family ------------------------------
split.vector <- factor(metadata[colnames(z_matrix), "groups"],
                       levels = c("Healthy_blood", "KP_blood", "Healthy_lung",
...
pdf(paste0(output_path, "chromVar_top200_all_names.pdf"),w=6, h = 12)
mat <- chromvar_mat[Top_chromVAR,]
col_fun = colorRamp2(c(-2, -1, 0 ,1, 2), c("#1F5FA9", "#74C4EA","white","#F4BA58","#A03124"))
...
ht <-Heatmap(t(scale(t(mat))), name = "Stand. \n chromVAR \n score",
             ...
             column_split = split.vector, top_annotation = colAnn, col = col_fun,
             show_column_names = F, column_title = NULL, cluster_columns = F,
             show_row_names = T, row_dend_gp = gpar(lwd = 0.5), row_names_gp = gpar(fontsize = 5))
draw(ht)
...
comp_df$TOBIAS_change <- footprinting_diff_res[comp_df$TF, "KP_lung_SiglecF_high_KP_lung_SiglecF_low_change"]
comp_df$TOBIAS_FC <- footprinting_diff_res[comp_df$TF, "KP_lung_SiglecF_high_mean_score"]/footprinting_diff_res[comp_df$TF, "KP_lung_SiglecF_low_mean_score"]
...
interesting_families <- c("STAT" , "NFAT-related", "SMAD", "bHLH-ZIP",#"More than 3 adjacent zinc fingers",
                          ...
                          "Maf-related", "Heteromeric CCAAT-binding", "E2F")
...
pdf(paste0(output_path, "chromVar_TOBIASchange-vs-deviation_SiglecFhiVSSiglecFlo.pdf"))
interesting_TFs <- c("Stat5a", "Stat5b", "Nfatc1", "Nfatc3", "Smad3", "Runx3", "Mef2d", "E2f4", "Atf3",
                     ...
                     "Stat4", "Stat3","Stat1", "Stat2", "Nfatc2", "Nfatc4", "Nfat5","Smad4")
comp_df_subset <- comp_df[which(comp_df$TF %in% interesting_TFs),]
comp_df_subset2 <- comp_df[which(comp_df$family.updated %in% names(family_colors) & comp_df$family.updated != "Other"),]
...
ggplot(comp_df, aes(Mean_diff, TOBIAS_FC, colour=family.updated, label=TF)) +
  ...
  # Bigger points for labeled TFs
  geom_point(
    data = unique(rbind(comp_df_subset2,comp_df_subset )),
    size = 3
  ) +
...
footprinting_diff_res <- read.table(footprinting_res_path2, header = T, sep = "\t")
rownames(footprinting_diff_res) <- sapply(footprinting_diff_res$motif_id, function(i) motifs_annot$mouse_gene_symbol[which(motifs_annot$name == i)])

comp_df$TOBIAS_change <- footprinting_diff_res[comp_df$TF, "KP_lung_SiglecF_high_Healthy_lung_change"]
comp_df$TOBIAS_FC <- footprinting_diff_res[comp_df$TF, "KP_lung_SiglecF_high_mean_score"]/footprinting_diff_res[comp_df$TF, "Healthy_lung_mean_score"]
comp_df$family <- sapply(comp_df$TF, function(i) motifs_annot[motifs_annot$mouse_gene_symbol == i, "tfclass_family"])
...
pdf(paste0(output_path, "chromVar_TOBIASchange-vs-deviation_SiglecFhiVSHealthyLung.pdf"))
interesting_TFs <- c("Stat5a", "Stat5b", "Nfatc1", "Nfatc3", "Smad3", "Runx3", "Mef2d", "E2f4", "Atf3",
                     "Tfeb", "Ctcf","Myc","Tfdp1","Znf143",
                     "Bcl6", "Cebpb", "Jun", "Junb", "Nfyc", "E2f1", "Fos", "Nfe2l2", "Bach1", "Maff", "Atf4", "Ddit3",
                     "Stat4", "Stat3","Stat1", "Stat2", "Nfatc2", "Nfatc4", "Nfat5","Smad4", "Smad5")
comp_df_subset <- comp_df[which(comp_df$TF %in% interesting_TFs),]
comp_df_subset2 <- comp_df[which(comp_df$family.updated %in% names(family_colors) & comp_df$family.updated != "Other"),]
