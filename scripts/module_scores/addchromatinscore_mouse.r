...
gene_body_gr <- get_gene_body_info(genes = all_genes$symbol, mart_obj = mouse, window_size = 5000)
length(unique(gene_body_gr$gene))

id_matching <- data.frame(entrez_ids = unique(unlist(all_genesets)),
                          symbols = mapIds(org.Mm.eg.db, keys = as.character(unique(unlist(all_genesets))), 
...
for(method in c("tss", "genebody", "nearestPeak")){
  ...
  senescence_genesets[["Senescence"]] <- senescence_genes$`Gene(murine)`
  ...
  module_scores_save$Pathway_peaks <- sapply(rownames(module_scores_save), function(i) length(pathways_peaks[[i]]))
  ...
  for(i in 1:length(comp_list)){
    ...
    names(contrast) <- c(sort(groups), paste0("batch", 2:5)) 
    ...
    # da_tests$Mean_diff <- sapply(da_tests$pathway, function(j) mean(module_scores[j, conditions == cond1]) - mean(module_scores[j, conditions == cond2]))
  ...
  for(pathway_of_interest in c("inflammatory response (GOBP)",
    ...
    cell_type_colors <- c(
      "Healthy_blood" = "#3288BD",
      "KP_blood" = "#70C4B4",
      "Healthy_lung" = "#FFD92F",
      "KP_lung_SiglecF_low" = "#FF7F00",
      "KP_lung_SiglecF_high" = "#D90017"
    )
    
    
    # Define the pairwise comparisons you want to annotate
    comparisons <- list(
      c("KP_lung_SiglecF_high", "Healthy_blood"),
      c("KP_lung_SiglecF_high", "KP_blood"),
      c("KP_lung_SiglecF_high", "Healthy_lung"),
      c("KP_lung_SiglecF_high", "KP_lung_SiglecF_low")
    )
